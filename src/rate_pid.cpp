...
void rate_roll_pid(float desired_roll, int speed) {
    ...
    derivative_roll = KD * ((error - last_error_roll) / dt); // Calculate the derivative term


    roll_correction = proportional_roll + (KI * integral_roll) + derivative_roll ; // Calculate the motor command based on PID output
...
void rate_roll_loop(int speed, float desired_roll) {
    ...
    ledcWrite(PWM_CHANNEL1, usToDuty(motor_side_b));
    ledcWrite(PWM_CHANNEL2, usToDuty(motor_side_a)); // Add a small offset to one motor to account for any imbalance
    ledcWrite(PWM_CHANNEL3, usToDuty(motor_side_a));
    ledcWrite(PWM_CHANNEL4, usToDuty(motor_side_b));
